...
class Collision {
    ...
    circleAndbox(node1, node2) {
        ...
        let x = node1Position[0];
        let y = node1Position[1];

        let p1 = [x, y - node1.height / 2];
        let p2 = [x, y + node1.height / 2];
        let p3 = [x - node1.width / 2, y];
        let p4 = [x + node1.width / 2, y];
        let ps = [p1, p2, p3, p4];
        for (let i = 0; i < ps.length; i++) {
            if (containPoint(ps[i], node2.borders)) {
                return true;
            }
        }
